...
public class PnlInfWp extends JPanel{
	...
	String[][] datos = {{"Cod WP","Name","Descripción","Budget","Fecha Inicio", "Fecha Fin"},
						{"id_wp","nombre","descripcion","presupuesto","f_ini","f_fin"}};
	String[] agrupar = {"Ninguno","Proyecto"};
	
	int dimension[] = {7,20,30,13,10,10};
	...
	public void generar_reporte(){
		...
		if(gbagrupa.getSelectedItem().equals("Ninguno")){
			...
			String consulta = "SELECT w.id_wp,w.nombre,w.descripcion,w.presupuesto,w.f_ini,w.f_fin FROM  WORKPAQUETS w" +
							" ORDER BY w."+datos[1][pos];
			...
			String consulta = "SELECT p.nombre AS proyecto, w.id_wp,w.nombre,w.descripcion,w.presupuesto,w.f_ini,w.f_fin FROM  WORKPAQUETS w" +
							" INNER JOIN PROYECTOS p ON p.id_pro = w.id_wp ORDER BY proyecto,w."+datos[1][pos];
