...
public class PnlInfTareas extends JPanel{
	...
	String[][] datos = {{"Cod Tarea","Name","description","Budget", "Fehca Inicio", "Fecha Fin"},
						{"id_task","nombre","descripcion","presupuesto","f_ini","f_fin"}};
	String[] agrupar = {"Ninguno","Work package"};
	
	int dimension[] = {7,20,30,12,10,10};
	...
	public void generar_reporte(){
		...
		if(gbagrupa.getSelectedItem().equals("Ninguno")){
			...
			*/
			...
			String consulta = "SELECT t.id_task,t.nombre,t.descripcion,t.presupuesto,t.f_ini,t.f_fin FROM TAREAS t " +
								"ORDER BY t."+datos[1][pos];
			...
			String consulta = "SELECT w.nombre AS WORK_PAKAGE,t.id_task,t.nombre,t.descripcion,t.presupuesto,t.f_ini,t.f_fin FROM TAREAS t" +
			" INNER JOIN WORKPAQUETS w ON t.id_wp = w.id_wp ORDER BY WORK_PAKAGE,t."+datos[1][pos];
			...
			try {
				dr = drb.build();
				drb.addColumn("WORKPAKAGE", "WORK_PAKAGE", String.class.getName(),20);
				pos=0;
				for(int i = 0; i<lista2.getModel().getSize();i++){
					//System.out.println(lista2.getModel().getElementAt(i));
					pos = buscar(datos,(String) lista2.getModel().getElementAt(i));
				...
				drb.addGroups(1);
				drb.setPrintBackgroundOnOddRows(false);
				drb.setTitle("TAREAS POR WORKPAKAGE");
				drb.setUseFullPageWidth(true);
			} catch (ColumnBuilderException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
